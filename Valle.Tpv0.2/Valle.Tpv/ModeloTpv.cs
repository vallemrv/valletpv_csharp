...
namespace Valle.TpvFinal
{
	public partial class ModeloTpv
	{
		...
		string nomImpresora = "caja2";
	    bool actualizar = false;
		bool sync = false;
        bool RutasMod = true;
		string UrlActualicacion = "";
		string UrlSync = "";
			...
			switch (accion)
            {
                ...
                case AccionesHerramientas.CambiarTpv:
                   ...
                   case AccionesHerramientas.Minimizar:
					...
					case AccionesHerramientas.ArqueoCaja:
						if(arqueoCaja==null){
							arqueoCaja = new ArqueoCaja(rutaDatos,nomImpresora);
					        arqueoCaja.Hidden+= delegate {  wSom.Hide(); };
					        arqueoCaja.SalirArqueoCaja += HandleArqueoCajaSalirArqueoCaja;	
						  }
				            else{
					          arqueoCaja.Show();
				           }
					...
					case AccionesHerramientas.MkClaves:
				       if(clave.Length>0 && esBloqueado){
						
									if(venClaves==null){
								         venClaves = new VenClaves();
								         venClaves.salirClaves+= HandleVenClavessalirClaves;
							         }else
								         venClaves.Show();
					         ...
					         herramientas.esBloqueado = esBloqueado;
					         herramientas.KeepAbove = true;
					         herramientas.Show();
				         }
        ...
        void HandleVenClavessalirClaves (object sender, EventArgClaves e)
        {
        	if(!e.estaCancelado){
			  if(e.clave == clave) esBloqueado = false;	
			   herramientas.esBloqueado = esBloqueado;
        ...
        void HandleArqueoCajaSalirArqueoCaja (object sender, ArqueoCajaArgs e)
        {
			if(!e.estaCancelado){
			   TpvFinal.DegloseArqueo desArq  = new TpvFinal.DegloseArqueo(e.monedas,idTpv,gestorBase);
				if(desArq.Permitido){
				    	ImpresoraTicket.ImprimirInforme(rutaDatos,nomImpresora,desArq.InformeCambio(),"Cambio remanente");
					    ImpresoraTicket.ImprimirInforme(rutaDatos,nomImpresora,desArq.InformeReintegro(),"Efentivo a retirar");
				}else{
		...
		void ReiniciarTpv(){
			...
			System.Diagnostics.Process restart = new System.Diagnostics.Process();
			      restart.StartInfo.FileName = Valle.Utilidades.RutasArchivos.Ruta_Principal+"/Restart.exe";
			      Console.WriteLine(restart.StartInfo.FileName);
