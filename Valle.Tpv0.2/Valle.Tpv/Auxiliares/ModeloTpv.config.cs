	...
	public partial class ModeloTpv
	{
		...
		void CargarTpvConfig(){
			...
			tbConfig.ReadXml(rutaPrincipal + "/" + RutasConfig.FICHERO_CONFIG);
			    ...
			    nomImpresora = tbConfig.Rows[0][RutasConfig.COL_NOM_IMP].ToString();
			    actualizar =  (bool)tbConfig.Rows[0][RutasConfig.COL_ACT];
			    sync =  (bool)tbConfig.Rows[0][RutasConfig.COL_SYNC];
			    UrlActualicacion = tbConfig.Rows[0][RutasConfig.COL_NOM_SERVIDOR_ACT].ToString();
			    UrlSync = tbConfig.Rows[0][RutasConfig.COL_NOM_SERVIDOR_SYNC].ToString();
    	}
			    ...
			    if(!ges.ContieneBaseDatos(dirBaseDatos)||ges.NumTablasBase(dirBaseDatos)<10){
			      Gtk.FileChooserDialog fc=
					new Gtk.FileChooserDialog("Fichero de esquema de datos",
						...
						filter.Name = "Esquema base tpv (*.sql)";
					...
					{
					    System.IO.StreamReader r = new System.IO.StreamReader(fc.Filename);
					    string sql = r.ReadToEnd();
					    foreach(string s in sql.Split(';')){
						   if(!String.IsNullOrEmpty(s.TrimEnd().TrimStart()))
						                             ges.EjScriptSql(s);
					     }
					    fc.Destroy();
					}
		...
		void ConfigurarTpv(){
			...
			if(actualizar) {
			  update = new Update(sqlComunciacion.pass
				                    ,sqlComunciacion.port,sqlComunciacion.usr,UrlActualicacion);
        ...
        private void iniciarClasesAux(int progreso)
        {
			...
			Gtk.Application.Invoke(delegate{
			  ...
			  tpv.mostrarVariosConNombre(this.MostrarVariosConNombre);
				...
				wSom.Colormap = wSom.Screen.RgbaColormap;
		...
		private void prepararBaseDatos()
        {
			...
			this.clave = rA["Bloqueado"].ToString().Length > 0 ?
            rA["Bloqueado"].ToString() : "";
		...
		void ComprobarPrimerUso(){
		  tbConfig = new DataTable(RutasConfig.NOMBRE_TB_CONFIG);
			   ...
			   if(System.IO.File.Exists(rutaPrincipal + "/" + RutasConfig.FICHERO_CONFIG))
				StartConfig();
			   else
